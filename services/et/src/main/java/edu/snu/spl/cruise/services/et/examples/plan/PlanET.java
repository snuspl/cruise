 ...
 */
package edu.snu.spl.cruise.services.et.examples.plan;
...
import static edu.snu.spl.cruise.services.et.examples.plan.PlanETDriver.NUM_INITIAL_EXECUTORS;


/**
 * Client code for Plan ET example.
 */
public final class PlanET {
  private static final Logger LOG = Logger.getLogger(PlanET.class.getName());

  private static final String DRIVER_IDENTIFIER = "Plan";

  // the number of associator + the number of subscriber + driver
  private static final int MAX_NUMBER_OF_EVALUATORS = NUM_INITIAL_EXECUTORS * 2 + 1;
  ...
  public static void main(final String[] args) throws InjectionException, IOException {
    final LauncherStatus status = runPlanET();
    LOG.log(Level.INFO, "ET job completed: {0}", status);
  }

  /**
   * Runs PlanET example app.
   * @throws InjectionException when fail to inject DriverLauncher
   */
  public static LauncherStatus runPlanET() throws InjectionException {
    ...
    final Configuration driverConfiguration = DriverConfiguration.CONF
        .set(DriverConfiguration.GLOBAL_LIBRARIES, EnvironmentUtils.getClassLocation(PlanETDriver.class))
