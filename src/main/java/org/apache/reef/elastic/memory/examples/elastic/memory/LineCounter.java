 ...
 * under the License.
 */
package org.apache.reef.elastic.memory.examples.elastic.memory;
...
/**
 * Driver side for the line counting demo that uses the elastic memory.
 */
@DriverSide
@Unit
      ...
      if (dataLoadingService.isDataLoadedContext(activeContext)) {
        ...
        activeContext.submitContext(Tang.Factory.getTang()
            .newConfigurationBuilder(ContextConfiguration.CONF
                .set(ContextConfiguration.IDENTIFIER, lcContextId).build())
            .build());
