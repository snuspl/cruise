...
/**
 * This optimizer assumes that all machines are homogeneous.
 ...
 * communication cost = Avg. # bytes of pull data per one mini-batch * No. of workers / No. of servers
...
public final class HomogeneousOptimizer implements Optimizer {
   ...
   * @param avgPullSize average number of pull data size per mini-batch
  ...
  private Pair<Double, Double> totalCost(final int numWorker,
    ...
    // Calculating commCost based on avg pull size per minibatch * max(#worker / #server, 1) / bandwidth * #minibatch
